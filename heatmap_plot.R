...
correlation_analysis <- function(data1, data2, prefix){
  ...
  write.table(result, file=paste(prefix, ".cor_pvalue.csv", sep=""), sep= ",")
  data.1<-scale(data1)
  data.2<-scale(data2)
  rvalue<-t(rvalue)
...
column_dend_height=unit(3,"cm"),
  ...
  p2 <- Heatmap(data.2,name="Metabolome",column_title = "Metabolome",column_title_side = "bottom",cluster_columns = F, cluster_rows = F,  width = unit(90, "cm"), height = unit(5, "cm"))
  lgd.list = Legend (col_fun = col_fun, title = "Microbiome")
  ht = p1%v%p2
  pdf("heat_p.pdf",width = 45, height = 10)
  draw(ht,annotation_legend_list = lgd.list,column_title ="Metabolome",column_title_side = "bottom",column_title_gp = gpar(fontface = 'bold', fontsize = 20),padding = unit(c(2, 2, 10, 2), "mm"))
  decorate_annotation("Microbiome", { 
    grid.text("Microbiome", gp = gpar(fontface = 'bold', fontsize = 20) ,y = unit(1, "npc") + unit(2, "mm"), just = "bottom") 
})
  dev.off()
  png("heat_p.png",width = 4000, height = 900)
  #draw(ht)
  draw(ht,annotation_legend_list = lgd.list,column_title ="Metabolome",column_title_side = "bottom",column_title_gp = gpar(fontface = 'bold', fontsize = 20),padding = unit(c(2, 2, 10, 2), "mm"))
  decorate_annotation("Microbiome", {
    grid.text("Microbiome",gp = gpar(fontface = 'bold', fontsize = 20) ,y = unit(1, "npc") + unit(2, "mm"), just = "bottom")
...
add_help_args <- function(args){
  ...
  if(length(args) != 3) {
    ...
    cat("Example:heatmap_plot.R env.tsv  Ingredients.tsv prefix\n")
    quit()
  }
}
