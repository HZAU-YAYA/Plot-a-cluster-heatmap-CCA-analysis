...
cca_plot<-function(data1,data2){
  ...
  p <- ggplot() + 
    ...
    ggsave("plot_cca.pdf", p, width = 135, height = 100, units="mm")
    ggsave("plot_cca.png", p, width = 135, height = 100, units="mm")
}
add_help_args <- function(args){
  ...
  if(length(args) != 2) {
    ...
    cat("Example:CCA_analysis.R MC_CO.genus.tsv MC_CO.differential_metabolites.tsv\n")
    quit()
  }
}
